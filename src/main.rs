...
use node::Node;
use shakmaty::{
    fen::Fen, CastlingMode, Chess, Color, EnPassantMode, File, Piece, Position, Rank, Role, Square,
};
...
fn add_to_previous_positions(new_fen: &Fen) {
    ...
    LAST_PROCESSED_FEN.set(Some(new_fen.clone())).unwrap();
}
...
async fn get_move(current_fen: String) -> String {
    ...
    match Some(fullmoves) == NonZeroU32::new(1) {
        true => {
            ...
            if bot_color == Color::White || condition {
                ...
                LAST_PROCESSED_FEN.set(None).unwrap();
    ...
    add_to_previous_positions(&fen_after_move);
...
struct SpritesByColor {
    white: Sprites,
    black: Sprites,
}

fn draw_piece(pos: Vec2, piece: &Piece, piece_sprites: &SpritesByColor, tile_size: f32) {
    let sprites = match piece.color {
        Color::White => &piece_sprites.black,
        Color::Black => &piece_sprites.white,
    };

    draw_texture_ex(
        match piece.role {
            Role::King => &sprites.king,
            Role::Queen => &sprites.queen,
            Role::Rook => &sprites.rook,
            Role::Bishop => &sprites.bishop,
            Role::Knight => &sprites.knight,
            Role::Pawn => &sprites.pawn,
        },
        pos.x * tile_size,
        pos.y * tile_size,
        WHITE,
...
fn draw_square(pos: Vec2, square: Square, tile_size: f32) {
    draw_rectangle(
        pos.x * tile_size,
        pos.y * tile_size,
        tile_size,
        tile_size,
        if square.is_dark() { WHITE } else { DARKGRAY },
    );
}
...
async fn main() {
    ...
    let sprites = SpritesByColor {
        white: Sprites {
            king: load_texture("/home/user/chessy/pieces/white/king.png")
                .await
                .unwrap(),
            queen: load_texture("/home/user/chessy/pieces/white/queen.png")
                .await
                .unwrap(),
            rook: load_texture("/home/user/chessy/pieces/white/rook.png")
                .await
                .unwrap(),
            bishop: load_texture("/home/user/chessy/pieces/white/bishop.png")
                .await
                .unwrap(),
            knight: load_texture("/home/user/chessy/pieces/white/knight.png")
                .await
                .unwrap(),
            pawn: load_texture("/home/user/chessy/pieces/white/pawn.png")
                .await
                .unwrap(),
        },
        black: Sprites {
            king: load_texture("/home/user/chessy/pieces/black/king.png")
                .await
                .unwrap(),
            queen: load_texture("/home/user/chessy/pieces/black/queen.png")
                .await
                .unwrap(),
            rook: load_texture("/home/user/chessy/pieces/black/rook.png")
                .await
                .unwrap(),
            bishop: load_texture("/home/user/chessy/pieces/black/bishop.png")
                .await
                .unwrap(),
            knight: load_texture("/home/user/chessy/pieces/black/knight.png")
                .await
                .unwrap(),
            pawn: load_texture("/home/user/chessy/pieces/black/pawn.png")
                .await
                .unwrap(),
        },
    };

    sprites.white.king.set_filter(FilterMode::Nearest);
    sprites.white.queen.set_filter(FilterMode::Nearest);
    sprites.white.rook.set_filter(FilterMode::Nearest);
    sprites.white.bishop.set_filter(FilterMode::Nearest);
    sprites.white.knight.set_filter(FilterMode::Nearest);
    sprites.white.pawn.set_filter(FilterMode::Nearest);

    sprites.black.king.set_filter(FilterMode::Nearest);
    sprites.black.queen.set_filter(FilterMode::Nearest);
    sprites.black.rook.set_filter(FilterMode::Nearest);
    sprites.black.bishop.set_filter(FilterMode::Nearest);
    sprites.black.knight.set_filter(FilterMode::Nearest);
    sprites.black.pawn.set_filter(FilterMode::Nearest);

    let tile_size = screen_width() / CHESS_FIELD_ROWS as f32;

    loop {
        clear_background(BLACK);

        for row in 0..CHESS_FIELD_ROWS {
            for column in 0..CHESS_FIELD_COLUMNS {
                draw_square(
                    vec2(row as f32, column as f32),
                    Square::from_coords(unsafe { transmute::<u8, File>(row as u8) }, unsafe {
                        transmute::<u8, Rank>(column as u8)
                    }),
                    tile_size,
                );
            }
