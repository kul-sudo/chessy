...
const getWeightOfPosition = (fen: string, botColor: 'w' | 'b'): number => {
  ...
  if (chessInstance.in_checkmate()) {
    ...
    const blackPiecesWeight =
      countInString(pieces, 'p') * PAWN_WEIGHT +
      countInString(pieces, 'r') * ROOK_WEIGHT +
      countInString(pieces, 'q') * QUEEN_WEIGHT +
      countInString(pieces, 'b') * BISHOP_WEIGHT +
      countInString(pieces, 'n') * KNIGHT_WEIGHT
    const whitePiecesWeight =
      countInString(pieces, 'P') * PAWN_WEIGHT +
      countInString(pieces, 'R') * ROOK_WEIGHT +
      countInString(pieces, 'Q') * QUEEN_WEIGHT +
      countInString(pieces, 'B') * BISHOP_WEIGHT +
